...
```mermaid
graph LR
    A["KIRI: Produk Dicintai (Product-Market Fit & Retention)"] -->|Fondasi Wajib Selesai Terlebih Dahulu| B["KANAN: Digunakan Banyak Pengguna (Scaling & Mass Adoption)"]
```
...
```mermaid
graph TD
    subgraph Step1 ["1. Segment & Persona"]
        A["Kriteria Pelanggan (Demografi, Geografi, Psikografi, Perilaku)"]
    end

    subgraph Step2 ["2. Goals & Jobs to be Done"]
        B["Goals: Apa tujuan akhir yang ingin dicapai?"]
        C["Jobs to be Done (JTBD): Kehendak/Pekerjaan yang harus dilakukan"]
    end

    subgraph Step3 ["3. Existing Solutions & Gap Analysis"]
        D["Existing Solutions: Alat/cara yang saat ini dipakai"]
        E["Hal yang DISUKAI dari Solusi Eksisting"]
        F["Hal yang TIDAK DISUKAI dari Solusi Eksisting (Root Pain)"]
    end

    subgraph Step4 ["4. Pains & Gains Identification"]
        G["Pains: Hambatan, ketakutan, kerugian waktu/uang"]
        H["Gains: Kemudahan, penghematan, ekspektasi lebih"]
    end
    ...
    C --> G
    C --> H
    F -->|Celah Peluang Terbesar / Golden Gap| I["Solusi Inovasi Startup"]
```
